    ...
    try:
        ...
        g.add(
            (
                datasetURI,
                OEO.publicationDate,
                Literal(metadatajson["publicationDate"], datatype=XSD.date),
            )
        )
        ...
        g.add(
            (datasetURI, DCAT.contactpoint, Literal(metadatajson["context"]["contact"]))
        )
        g.add(
            (
                datasetURI,
                OEO.documentation,
                Literal(metadatajson["context"]["documentation"]),
            )
        )
        g.add(
            (datasetURI, OEO.sourceCode, Literal(metadatajson["context"]["sourceCode"]))
        )
        ...
        g.add(
            (
                ...
                Literal(metadatajson["spatial"]["resolution"]),
            )
        )
        ...
        if "temporal" in metadatajson:
            ...
            g.add(
                (
                    ...
                    Literal(metadatajson["temporal"]["resolution"]),
                )
            )
            g.add(
                (
                    t,
                    OEO.referenceDate,
                    Literal(metadatajson["temporal"]["referenceDate"]),
                )
            )
        ...
        # licenses
        licdict = {"ODbL-1.0": "https://www.dcat-ap.de/def/licenses/odbl"}
        for l in metadatajson["licenses"]:
            ...
            g.add(
                (
                    ...
                    Literal(l["instruction"] + "\t" + l["attribution"]),
                )
            )
            li = BNode()
        ...
        for d in metadatajson["resources"]:
            d_uri = URIRef(
                "http://openenergy-platform.org/ontology/v0.0.1/oeo/" + d["name"]
            )
            ...
            g.add(
                (foreignKey, OEO.fields, Literal(d["schema"]["foreignKeys"]["fields"]))
            )
            reference = BNode()
            ...
            g.add(
                (
                    reference,
                    OEO.ressource,
                    Literal(d["schema"]["foreignKeys"]["reference"]["ressource"]),
                )
            )
            g.add(
                (
            ...
            g.add(
                (
                    ...
                    Literal(d["dialect"]["decimalSeparator"]),
                )
            )
        ...
        g.add(
            (
                datasetURI,
                OEO.metadataVersion,
                Literal(metadatajson["metaMetadata"]["metadataVersion"]),
            )
        )
        metalicense = BNode()
        ...
        g.add(
            (
                metalicense,
                DCTERMS.title,
                Literal(metadatajson["metaMetadata"]["metadataLicense"]["name"]),
            )
        )
        g.add(
            (
                metalicense,
                DCTERMS.title,
                Literal(metadatajson["metaMetadata"]["metadataLicense"]["title"]),
            )
        )
        g.add(
            (
                metalicense,
                OEO.path,
                Literal(metadatajson["metaMetadata"]["metadataLicense"]["path"]),
            )
        )
